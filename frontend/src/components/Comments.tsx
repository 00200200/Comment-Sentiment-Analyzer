...
export default function Comments({ videoId }: CommentsProps) {
	...
	const filteredComments = sortedComments.filter(comment => comment.text.toLowerCase().includes(keyword.toLowerCase()));
	const sentimentCounts = filteredComments.reduce(
		(acc, comment) => {
			switch (comment.sentiment_label) {
				...
				case 'neutral':
					...
					break;
				default:
					// zakładamy "ambiguous" lub cokolwiek innego
					acc.ambiguous += 1;
			}
			return acc;
		},
	...
	const paginatedComments = filteredComments.slice((page - 1) * COMMENTS_PER_PAGE, page * COMMENTS_PER_PAGE);
		...
		<section className='mt-8'>
			...
			<div className='flex justify-between items-center mb-4'>
				...
				<div className='flex items-center gap-4'>
					<span className='text-gray-500 text-sm'>{filteredComments.length} comments found</span>
					<Popover>
			...
			{!isLoading && !error && filteredComments.length === 0 && <p>No comments available.</p>}
					...
					{showPagination && filteredComments.length > COMMENTS_PER_PAGE && (
						<Pagination className='mt-2'>
							<PaginationContent>
								<PaginationItem>
									<PaginationPrevious
										onClick={page > 1 ? () => setPage(prev => Math.max(prev - 1, 1)) : undefined}
								...
								<PaginationItem>
									<PaginationNext
										onClick={
											page * COMMENTS_PER_PAGE < filteredComments.length ? () => setPage(prev => prev + 1) : undefined
										}
										className={
											page * COMMENTS_PER_PAGE >= filteredComments.length ? 'opacity-50 pointer-events-none' : ''
										}
									/>
								</PaginationItem>
							</PaginationContent>
